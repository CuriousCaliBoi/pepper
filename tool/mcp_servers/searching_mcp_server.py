...
from pepper.tool.utils.smart_request import request_to_json, smart_request
...
async def search_wiki_revision(
    ...
    try:
        ...
        if "revisions" not in page_info or not page_info["revisions"]:
            return (
                adjustment_msg
                + f"Page Title: {page_title}\n\nNo revisions found for '{entity}' in {year}-{month:02d}.\n\nThe page may not have been edited during this time period."
            )
...
async def search_archived_webpage(url: str, year: int, month: int, day: int) -> str:
    ...
    try:
        ...
        return f"[ERROR]: Failed to connect to Wayback Machine: {e}"
    except ValueError as e:
        return (
            f"[ERROR]: Data Error: Failed to parse response from Wayback Machine: {e}"
        )
    except Exception as e:
        return f"[ERROR]: Unexpected Error: An unexpected error occurred: {e}"
